...
/**
 * A Builder class for a Retrofit REST Adapter. Extends the default implementation by providing logic to handle an OAuth 2.0 password grant
 * login flow. The RestAdapter that it produces uses an interceptor to automatically obtain a bearer token from the authorization server and
 * insert it into all client requests.
 * 
 * You can use it like this:
 * 
 * private VideoSvcApi videoService = new SecuredRestBuilder() .setLoginEndpoint(TEST_URL + VideoSvcApi.TOKEN_PATH) .setUsername(USERNAME)
 * .setPassword(PASSWORD) .setClientId(CLIENT_ID) .setClient(new ApacheClient(UnsafeHttpsClient.createUnsafeClient()))
 * .setEndpoint(TEST_URL).setLogLevel(LogLevel.FULL).build() .create(VideoSvcApi.class);
 * 
...
public class SecuredRestBuilder extends RestAdapter.Builder {
    ...
    private class OAuthHandler implements RequestInterceptor {
        ...
        public OAuthHandler(Client client, String tokenIssuingEndpoint, String username, String password, String clientId,
                String clientSecret) {
            super();
            this.client = client;
        ...
        /**
         * Every time a method on the client interface is invoked, this method is going to get called. The method checks if the client has
         * previously obtained an OAuth 2.0 bearer token. If not, the method obtains the bearer token by sending a password grant request to
         * the server.
         * 
         * Once this method has obtained a bearer token, all future invocations will automatically insert the bearer token as the
         * "Authorization" header in outgoing HTTP requests.
         * 
         */
        @Override
